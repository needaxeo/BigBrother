...
public class BBPlayerListener extends PlayerListener {
	...
	public void onPlayerJoin(PlayerJoinEvent event) {
		...
		/*
		 * if (!plugin.haveSeen(player)) { plugin.markSeen(player); if
		 * (BBSettings.autoWatch) { plugin.watchPlayer(player); } }
		 */
	...
	/*
	 * 
	 * public void onBlockRightClick(BlockRightClickEvent event) { Player player
	 * = event.getPlayer(); if (BBPermissions.info(player) &&
	 * plugin.hasStick(player, player.getItemInHand()) &&
	 * plugin.rightClickStick(player)) { plugin.stick(player, event.getBlock());
	...
	public void onPlayerInteract(PlayerInteractEvent event) {
		...
		if (event.getAction().equals(Action.LEFT_CLICK_BLOCK)) {
			if (BBPermissions.info(player) && plugin.hasStick(player, player.getItemInHand()) && plugin.leftClickStick(player)) {
				...
				event.setCancelled(true); // Cancel in case of 1-hit breakable
											// stuff like flowers.
			}
		}
			...
			if (BBPermissions.info(player) && plugin.hasStick(player, player.getItemInHand()) && plugin.rightClickStick(player)) {
				...
				case BUCKET:
					...
					case LAVA:
						...
						type = Material.LAVA.getId();
						dataBlock2 = new BrokenBlock(BBUsersTable.getInstance().getUserByName(event.getPlayer().getName()),
								world.getName(), x, y, z, type, (byte) 0);
						dataBlock2.send();
						break;
				...
				default:
					...
					case LEVER:
						if (BBSettings.leverSwitch) {
							LeverSwitch leverDataBlock = new LeverSwitch(event.getPlayer().getName(), block, block.getWorld()
									.getName());
							leverDataBlock.send();
						}
						break;
					case STONE_BUTTON:
						if (BBSettings.buttonPress) {
							ButtonPress buttonDataBlock = new ButtonPress(event.getPlayer().getName(), block, block.getWorld()
									.getName());
					...
					case CHEST:
						if (BBSettings.chestChanges) {
							BBDataBlock chestDataBlock = new ChestOpen(event.getPlayer().getName(), block, block.getWorld()
									.getName());
							chestDataBlock.send();
						}
						break;
